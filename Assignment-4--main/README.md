...
docker run -p 3000:3000 --rm --name docker-demo-app docker-demo-app:latest
```
...
Docker containers are lightweight, process-isolated environments that share the host OS kernel, while virtual machines (VMs) include a full guest OS with its own kernel on top of a hypervisor. This makes containers much smaller and faster to start, use fewer resources, and pack more densely on the same hardware. For development teams, Docker simplifies dependency management, provides consistent runtime environments across machines (dev, CI, prod), and speeds up CI/CD pipelines. For apps like this Node.js server, containers make it easy to reproduce the runtime, scale instances, and deploy updates with minimal overhead compared to VMs.
...
To build and start the container with Docker Compose:
